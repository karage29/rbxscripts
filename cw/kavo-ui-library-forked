...
function Kavo.CreateLib(kavName, themeList)
	...
	function Tabs:NewTab(tabName)
		...
		function Sections:NewSection(secName, hidden)
			...
			function Elements:NewSlider(slidInf, slidTip, maxvalue, minvalue, callback, startVal, step)
				...
				local Value = startVal or nil
				if Value then
					val.Text = Value
					local range = tonumber(maxvalue) - tonumber(minvalue)
					local relative = (Value - tonumber(minvalue)) / range
					local initialX = relative * 149 -- Use 149 or sliderBtn.AbsoluteSize.X if sized already
					sliderDrag.Size = UDim2.new(0, math.clamp(initialX, 0, 149), 1, 0)
				end
				sliderBtn.MouseButton1Down:Connect(function()
					if not focusing then
						...
						-- Calculate and snap to 0.1 step
						local relative = math.clamp((mouse.X - sliderPos) / 149, 0, 1)
						local rawValue = relative * range + tonumber(minvalue)
						Value = tonumber(minvalue) + math.round((rawValue - tonumber(minvalue)) / step) * step
						Value = math.clamp(Value, tonumber(minvalue), tonumber(maxvalue)) -- Ensure bounds
						local displayValue = math.floor(Value * 10 + 0.5) / 10
						local isWhole = (math.floor(displayValue) == displayValue)
						val.Text = isWhole and tostring(math.floor(displayValue)) or string.format("%.1f", displayValue)
						pcall(function()
							callback(Value)
						end)
						local targetWidth = ((Value - tonumber(minvalue)) / range) * 149
						sliderDrag.Size = UDim2.new(0, math.clamp(targetWidth, 0, 149), 1, 0)
						...
						moveconnection = mouse.Move:Connect(function()
							...
							local displayValue = math.floor(Value * 10 + 0.5) / 10
							local isWhole = (math.floor(displayValue) == displayValue)
							val.Text = isWhole and tostring(math.floor(displayValue))
								or string.format("%.1f", displayValue)
							pcall(function()
								callback(Value)
							end)
							local targetWidth = ((Value - tonumber(minvalue)) / range) * 149
